...
#define UVC_INTF_STREAMING_EXT 2 //Streaming two
...
#ifdef BULK_MODE
	void (*encode)(struct usb_request *req, struct uvc_video *video, struct usbd_uvc_buffer *buf);
#else
...
//typedef void*	_lock;
#define UVC_NUMBER 2 //16//2
#define UVC_STREAM_ONE 0
#define UVC_STREAM_TWO 1
struct uvc_dev
{
	struct uvc_common *common;
	struct uvc_video video[UVC_NUMBER];
...
#ifdef BULK_MODE
static unsigned int streaming_maxpacket = 512; //512;//1000;//1024;
#else
static unsigned int streaming_maxpacket = 1000;
#endif
...
#define UVC_INTF_VIDEO_STREAMING_EXT 2
...
extern struct usb_endpoint_descriptor uvc_fs_streaming_ep_ext;
extern struct usb_endpoint_descriptor uvc_hs_streaming_ep_ext;
...
extern struct usb_interface_descriptor uvc_streaming_intf_alt2;
extern struct usb_interface_descriptor uvc_streaming_intf_two_alt0;
extern struct usb_interface_descriptor uvc_streaming_intf_two_alt1;
DECLARE_UVC_INPUT_HEADER_DESCRIPTOR(1, 4);
extern struct UVC_INPUT_HEADER_DESCRIPTOR(1, 4) uvc_input_header;
extern struct UVC_INPUT_HEADER_DESCRIPTOR(1, 4) uvc_input_header_ext;
extern struct UVC_INPUT_HEADER_DESCRIPTOR(1, 4) uvc_input_two_header;
...
extern struct uvc_descriptor_header *uvc_hs_streaming_cls_ext[];
extern struct uvc_descriptor_header *uvc_fs_control_cls[];

DECLARE_UVC_HEADER_DESCRIPTOR(2); //Devin
extern struct UVC_HEADER_DESCRIPTOR(2) uvc_control_header;
...
extern struct uvc_output_terminal_descriptor uvc_output_ext_terminal;
...
extern struct uvc_format_info uvc_formats_ext[];
/////////////////////////
...
#else
void uvc_video_put_out_stream_queue(struct usbd_uvc_buffer *payload, struct uvc_video *video);
void uvc_video_put_in_stream_queue(struct usbd_uvc_buffer *payload, struct uvc_video *video);
struct usbd_uvc_buffer *uvc_video_out_stream_queue(struct uvc_video *video);
#endif
