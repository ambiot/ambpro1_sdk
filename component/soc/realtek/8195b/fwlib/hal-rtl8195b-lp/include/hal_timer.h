 ...
 * @ingroup 8195bl_hal
 * @{
 * @brief The TIMER HAL module of the LS platform.
 */

#if (CHIP_VER == CHIP_A_CUT)
u32 hal_timer_set_timeout_rtl8195blp_patch (phal_timer_adapter_t ptimer_adp, u32 time_us, u32 res_us);
void hal_timer_start_periodical_rtl8195blp_patch (phal_timer_adapter_t ptimer_adp, u32 time_us,
...
uint32_t hal_timer_set_timeout (phal_timer_adapter_t ptimer_adp, uint32_t time_us, uint32_t res_us)
{
    ...
    return hal_timer_set_timeout_rtl8195blp_patch (ptimer_adp, time_us, res_us);
#else
...
{
    hal_gtimer_stubs.hal_timer_disable_match_event (ptimer_adp, match_ev);
}
...
{
    #if (CHIP_VER == CHIP_A_CUT) && (defined(CONFIG_BUILD_RAM))
        hal_timer_start_periodical_rtl8195blp_patch(ptimer_adp, time_us, callback, phid);
    #else
...
void hal_start_systimer(phal_timer_adapter_t ptimer_adp, uint32_t tmr_id,
                        timer_cnt_mode_t cnt_md, uint32_t tick_time)
{
    hal_gtimer_stubs.hal_start_systimer (ptimer_adp, tmr_id, cnt_md, tick_time);
}
