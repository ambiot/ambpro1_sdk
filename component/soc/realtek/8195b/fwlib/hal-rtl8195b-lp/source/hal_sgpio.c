...
extern void hal_syson_sgpio_pin_ctrl(u8 pin_sel, BOOL en);
extern void hal_syson_sgpio_rst(void);


/**
 * @addtogroup ls_hal_qdec SGPIO
 * @{
 */
...
{
    hal_sgpio_stubs.hal_sgpio_irq_reg (handler);
}
...
{
    hal_sgpio_stubs.hal_sgpio_unreg ();
}
...
HAL_Status hal_sgpio_init (hal_sgpio_adapter_t *psgpio_adp, sgpio_pin_sel_lp_t pin_sel)
{
    ...
    pin_list = (io_pin_t *)&hal_sgpio_stubs.psgpio_pin[pin_sel * 3];
    ret = hal_pinmux_register (pin_list, PID_SGPIO0);
    if (ret == HAL_OK) {
        return hal_sgpio_stubs.hal_sgpio_init (psgpio_adp, pin_sel);
    } else {
        return ret;
    }
}
...
void hal_sgpio_deinit (hal_sgpio_adapter_t *psgpio_adp)
{
    ...
    pin_list = (io_pin_t *)&hal_sgpio_stubs.psgpio_pin[sgpio_pin_sel * 3];
    hal_pinmux_unregister (pin_list, PID_SGPIO0);
}

/**
 *  @brief Release the SGPIO pin after initialize the SGPIO.
 *
 *  @param[in]  psgpio_adp The SGPIO adapter.
 *
 *  @returns void
...
void hal_sgpio_pin_free (hal_sgpio_adapter_t *psgpio_adp)
{
    ...
    hal_syson_sgpio_pin_ctrl(sgpio_pin_sel, DISABLE);
    pin_list = (io_pin_t *)&hal_sgpio_stubs.psgpio_pin[sgpio_pin_sel * 3];
    hal_pinmux_unregister (pin_list, PID_SGPIO0);
}
...
HAL_Status hal_sgpio_dma_match_output_init (hal_sgpio_adapter_t *psgpio_adp, hal_gdma_adaptor_t *pgdma_adaptor, hal_sgpio_dma_match_output_t *pdma_match_output)
{
        ...
        ret = hal_gdma_chnl_alloc (pgdma_adaptor);    // default no-multiple block support
