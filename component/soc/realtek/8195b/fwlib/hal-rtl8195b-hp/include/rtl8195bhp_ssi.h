/**************************************************************************//**
 * @file     rtl8195bhp_ssi.h
 * @brief    The header file of rtl8195bhp_ssi.c.
 * @version  1.00
 * @date     2017-08-22
 *
 * @note
...
#ifndef RTL8195BHP_SSI_H
#define RTL8195BHP_SSI_H

/**

        \addtogroup hs_hal_ssi SPI
        \ingroup 8195bh_hal
        \brief The Serial Peripheral Interface bus(SPI) module of the HS platform.
        @{
*/
...
#define MAX_SPI_NUM        0x4          //!< Define maximum SPI device numbers, 4 devices in our platform at most.
#define RTK_SLAVE_INDEX    0x3          //!< Define high speed SPI slave index
...
#define BIT_IMR_FAEIM  BIT5             //!< Define SPI interrupt mask bit field of FAEIM, frame alignment error.
...
/**
  \brief  Enumeration to define SPI pinmux selection
*/
enum spi_pin_sel_e {
    S0 = 0,                             //!< Select S0 pinmux
    S1 = 1                              //!< Select S1 pinmux
};
typedef uint8_t spi_pin_sel_t;
...
typedef struct hal_ssi_adaptor_s {
    ...
    spi_pin_sel_t                   pin_sel;                            //!< The pinmux selection of the SPI device
...
/**
  \brief  The data struct of SPI group, this group is managing IRQ handles and the interrupt signal
*/
typedef struct _hal_ssi_group_s {
    phal_ssi_adaptor_t phal_ssi_adaptor[MAX_SPI_NUM];                   //!< The pointer stores pointers of each SPI device adaptor
    u8 spi_in_use;                                                      //!< A flag shows if any SPI device is used
    u8 irq_en;                                                          //!< A flag indicates if IRQ is enabled
} hal_ssi_group_t, *phal_ssi_group_t;
...
typedef struct hal_ssi_func_stubs_s {
    phal_ssi_group_t phal_ssi_group;
    uint8_t *ssi_pin;
    hal_status_t (*hal_ssi_clock_ctl) (phal_ssi_adaptor_t phal_ssi_adaptor, u8 ctl);
    hal_status_t (*hal_ssi_pin_ctl) (phal_ssi_adaptor_t phal_ssi_adaptor, spi_pin_sel_t pin_sel, u8 ctl);
...
hal_status_t hal_ssi_clock_ctl_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, u8 ctl);
hal_status_t hal_ssi_pin_ctl_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, spi_pin_sel_t pin_sel, u8 ctl);
hal_status_t hal_ssi_pinmux_disable_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_enable_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_disable_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_init_setting_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_deinit_setting_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
void hal_ssi_read_interrupt_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
void hal_ssi_write_interrupt_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
void hal_ssi_irq_handle_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_interrupt_enable_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_interrupt_disable_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_interrupt_init_read_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, void *rx_data, u32 length);
hal_status_t hal_ssi_interrupt_init_write_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, void *tx_data, u32 length);
hal_status_t hal_ssi_set_sclk_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, u32 clk_rate);
hal_status_t hal_ssi_set_format_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_set_microwire_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_set_sclk_polarity_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, u8 sclk_polarity);
hal_status_t hal_ssi_set_sclk_phase_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, u8 sclk_phase);
hal_status_t hal_ssi_set_data_frame_number_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, u32 length);
hal_status_t hal_ssi_set_interrupt_mask_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, u8 imr_value);
hal_status_t hal_ssi_set_device_role_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, u32 role);
hal_status_t hal_ssi_set_txfifo_threshold_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, u32 txftlr_value);
hal_status_t hal_ssi_set_rxfifo_threshold_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, u32 rxftlr_value);
hal_status_t hal_ssi_set_slave_enable_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, u32 slave_index);
u32 hal_ssi_get_rxfifo_level_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
u32 hal_ssi_get_txfifo_level_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
u32 hal_ssi_get_status_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
u32 hal_ssi_get_busy_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
u32 hal_ssi_get_interrupt_mask_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
u32 hal_ssi_get_interrupt_status_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
u32 hal_ssi_get_raw_interrupt_status_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
u32 hal_ssi_get_slave_enable_register_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
u32 hal_ssi_writable_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
u32 hal_ssi_readable_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_write_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, u32 value);
u32 hal_ssi_read_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_is_timeout_rtl8195bhp(u32 start_count, u32 timeout_count);
hal_status_t hal_ssi_stop_recv_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_enter_critical_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_exit_critical_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor);
void hal_ssi_tx_gdma_irq_handle_rtl8195bhp (phal_ssi_adaptor_t phal_ssi_adaptor);
void hal_ssi_rx_gdma_irq_handle_rtl8195bhp (phal_ssi_adaptor_t phal_ssi_adaptor);
hal_status_t hal_ssi_tx_gdma_init_setting_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor,phal_gdma_adaptor_t phal_gdma_adaptor);
hal_status_t hal_ssi_rx_gdma_init_setting_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor,phal_gdma_adaptor_t phal_gdma_adaptor);
hal_status_t hal_ssi_dma_send_init_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, u8 *ptx_data, u32 length);
hal_status_t hal_ssi_dma_recv_init_rtl8195bhp(phal_ssi_adaptor_t phal_ssi_adaptor, u8 *prx_data, u32 length);
