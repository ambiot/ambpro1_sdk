...
#ifndef RTL8195BHP_SCE_H
#define RTL8195BHP_SCE_H
...
/** @addtogroup Device_SCE The Secure Code Engine registers.
  * @ingroup Device_Peripheral_peripherals
  * @{
  */
...
#if (defined(CONFIG_BUILD_LIB) && (CONFIG_BUILD_LIB==1)) || (defined(CONFIG_BUILD_ALL) && (CONFIG_BUILD_ALL==1))
#if CHIP_VER < CHIP_C_CUT
...
#else
// for C-Cut(and after) chip
enum sce_page_size_e {
    Page64KB            = 0,    ///< page size = 64K bytes
    Page128KB           = 1,    ///< page size = 128K bytes
    Page256KB           = 2     ///< page size = 256K bytes
};
#endif
#endif
typedef uint8_t sce_page_size_t;

enum sce_size_byte_e {
    Size16KB            = 1024*16,    ///< 16K bytes
    Size32KB            = 1024*32,    ///< 32K bytes
    Size64KB            = 1024*64,    ///< 64K bytes
    Size128KB           = 1024*128,   ///< 128K bytes
    Size256KB           = 1024*256    ///< 256K bytes
};
...
void hal_sce_write_reg_rtl8195bhp (uint8_t memory_type, uint32_t addr, uint32_t value);
uint32_t hal_sce_read_reg_rtl8195bhp (uint8_t memory_type, uint32_t addr);
uint8_t hal_sce_comm_alloc_section_rtl8195bhp (sce_mem_type_t memory_type);
void hal_sce_comm_free_section_rtl8195bhp (sce_mem_type_t memory_type, uint8_t sec_id);
BOOLEAN hal_sce_comm_key_valid_rtl8195bhp (sce_mem_type_t memory_type, uint8_t pair_id);
void hal_sce_comm_set_key_valid_rtl8195bhp (sce_mem_type_t memory_type, uint8_t pair_id);
hal_status_t hal_sce_func_enable_rtl8195bhp (sce_mem_type_t memory_type);
hal_status_t hal_sce_func_disable_rtl8195bhp (sce_mem_type_t memory_type);
hal_status_t hal_sce_enable_rtl8195bhp (sce_mem_type_t memory_type, sce_page_size_t page_size,
                                        sce_block_size_t block_size, sce_mode_select_t rw_mode);
hal_status_t hal_sce_disable_rtl8195bhp (sce_mem_type_t memory_type);
void hal_sce_set_key_rtl8195bhp (sce_mem_type_t memory_type, uint8_t pair_id, uint8_t *key);
void hal_sce_set_iv_rtl8195bhp (sce_mem_type_t memory_type, uint8_t pair_id, uint8_t *iv);
void hal_sce_set_key_pair_rtl8195bhp (sce_mem_type_t memory_type, uint8_t pair_id, uint8_t *key, uint8_t *iv);
void hal_sce_read_key_pair_rtl8195bhp (sce_mem_type_t memory_type, uint8_t pair_id, uint8_t *key, uint8_t *iv);
uint8_t hal_sce_key_pair_search_rtl8195bhp (sce_mem_type_t memory_type, uint8_t *key, uint8_t *iv);
hal_status_t hal_sce_set_section_rtl8195bhp (sce_mem_type_t memory_type, uint8_t sec_id, uint8_t pair_id,
                                    uint32_t start_address, uint32_t end_address, uint8_t secure_en);
void hal_sce_section_disable_rtl8195bhp (sce_mem_type_t memory_type, uint8_t sec_id);
hal_status_t hal_sce_flash_remap_rtl8195bhp (uint32_t phy_addr, uint32_t vir_addr, uint32_t map_size,
                                             sce_page_size_t page_size_sel, uint8_t secure_en,
                                             sce_block_size_t block_size, uint8_t key_id);
hal_status_t hal_sce_set_mem_crypto_rtl8195bhp (sce_mem_type_t memory_type, uint32_t start_addr,
                                                uint32_t mem_size, uint8_t key_id);
hal_status_t hal_sce_remap_enable_rtl8195bhp (sce_mem_type_t memory_type, uint8_t sec_id, uint32_t remap_addr);
void hal_sce_reg_dump_rtl8195bhp (uint32_t type);
hal_status_t hal_sce_cfg_rtl8195bhp (sce_mem_type_t memory_type, sce_page_size_t page_size,
                                     sce_block_size_t block_size, sce_mode_select_t rw_mode);
...
typedef struct hal_sce_func_stubs_s {
    ...
    hal_status_t (*hal_sce_enable) (sce_mem_type_t memory_type, sce_page_size_t page_size,
                                    sce_block_size_t block_size, sce_mode_select_t rw_mode);
