 ...
 *   \return hal_status_t.
 */
__STATIC_INLINE hal_status_t hal_ssi_clock_ctl (phal_ssi_adaptor_t phal_ssi_adaptor, u8 ctl)
{
    return hal_ssi_stubs.hal_ssi_clock_ctl (phal_ssi_adaptor, ctl);
}

/** \brief Description of hal_ssi_pin_ctl
 *
 *    hal_ssi_pin_ctl is used to enable and select pinmux for the target SPI device.
 *
 *   \param phal_ssi_adaptor_t phal_ssi_adaptor:      The pointer of SPI adaptor.
 *   \param spi_pin_sel_t pin_sel:      The pinmux selection for the device.
 *   \param u8 ctl:      A control bit to decide to enable or disable the pinmux of the device.
 *
 *   \return hal_status_t.
 */
__STATIC_INLINE hal_status_t hal_ssi_pin_ctl (phal_ssi_adaptor_t phal_ssi_adaptor, spi_pin_sel_t pin_sel, u8 ctl)
{
    return hal_ssi_stubs.hal_ssi_pin_ctl (phal_ssi_adaptor, pin_sel, ctl);
}
