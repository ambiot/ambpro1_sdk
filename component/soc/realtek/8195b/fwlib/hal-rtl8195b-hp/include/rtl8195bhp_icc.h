 ...
 * @ingroup 8195bh_hal
 * @{
 * @brief The ICC(Inter CPU Communication) HAL on HS platform.
 *        The ICC implements a mechanism for communication between the HS platform and the LS platform.
 */

#ifndef RTL8195BHP_ICC_H
#define RTL8195BHP_ICC_H

#ifdef __cplusplus
extern "C" {
#endif

#include "rtl8195bhp_icc_h_type.h"
...
/*! Defines the timeout period for the waiting of response of LS
    platform memory/register read command. */
#define ICC_LS_READ_CMD_TIMEOUT_US          (5000000)
...
/**
  \brief  The call back function type for the indication of L2H message
          queue is updated.
*/
typedef void (*icc_q_ind)(hal_icc_adapter_t *para, uint32_t q_id);
...
/**
  \brief  The function type of a ICC user application message handler.
*/
...
struct icc_msg_tx_req_s {
    ...
    icc_msg_tx_cb_t pnscallback;    ///< the NS domain callback function for the message transmission is done
    uint32_t nscallback_arg;          ///< the argument of the NS domain callback function
#endif    
};
...
/**
  \brief  The data type for the ICC entity HAL control and management.
*/
struct hal_icc_adapter_s {
    union {
        ...
        struct {
            ...
            volatile uint32_t h2l_cmd_busy:1;   ///< indication of the H2L command is busy
    ...
    icc_q_rw_req_t h2l_hq_wr_req; ///< to store the writing request to the H2L_HQ, for interrupt mode writing
    icc_q_rw_req_t h2l_lq_wr_req; ///< to store the writing request to the H2L_LQ, for interrupt mode writing
    icc_q_rw_req_t l2h_hq_rd_req; ///< to store the reading request to the L2H_HQ, for interrupt mode reading
    icc_q_rw_req_t l2h_lq_rd_req; ///< to store the reading request to the L2H_LQ, for interrupt mode reading
    ...
    icc_cmd_ind l2h_cmd_ind;    ///< callback function for a new L2H command indication
    void *l2h_cmd_ind_cb_para;  ///< the application data will be pass back with the callback function of new L2H command indication
    icc_q_ind l2h_msg_q_ind;    ///< callback function for the indication of L2H message queue is updated(L2H_Q write pointer is updated).
    void *l2h_msg_q_ind_cb_para;   ///< the application data will be pass back with the callback function of message queue is updated.
    icc_q_ack h2l_msg_q_ack;    ///< callback function for a H2L message queue is acknowledged(H2L_Q read pinter is updated).
    void *h2l_msg_q_ack_cb_para;   ///< the application data will be pass back with the callback function of message queue is updated.
    void (*dcache_invalidate_by_addr)(uint32_t *addr, int32_t dsize);   ///< the D-cache invalidate function
    void (*dcache_clean_by_addr) (uint32_t *addr, int32_t dsize);   ///< the D-cache clean function
    volatile uint32_t ls_mem_rd_pbuf;   ///<  the buffer address for LS memory/register reading
    volatile uint32_t ls_mem_rd_cnt;   ///< the size of data(in double word) is waiting from LS, for LS memory reading command
    ...
    uint32_t ls_read_timeout_us;                    ///< the timeout period(in us) for waiting of LS platform memory/register reading response
...
// B-Cut
    ...
    icc_msg_tx_req_t *free_tx_req_h;             ///< the message queue TX request pool head, this pool should be filled when ICC HAL initialization
    icc_msg_tx_req_t *free_tx_req_t;             ///< the message queue TX request pool tail
    
    volatile uint32_t ls_op_ret;                 ///< the return status for ICC command to LS
    volatile uint32_t ls_op_data;                ///< the return data of a ICC command from LS
};

/**
  \brief  The data type of the ICC message queue.
*/
typedef struct icc_msg_queue_s {
    uint32_t q_write_ptr;       ///< the message queue data write pointer
    uint32_t q_read_ptr;        ///< the message queue data read pointer
    uint32_t q_size;            ///< the message queue size
    uint32_t *q_start_addr;     ///< the buffer address of this message queue
} icc_msg_queue_t, *picc_msg_queue_t;

/**
  \brief  Defines the ICC message queue ID.
*/
enum icc_msg_qid_e {
    QID_H2L_HQ          = 0,
    QID_H2L_LQ          = 1,
...
/* Defines the ICC registers are used for the implementation of the ICC HS to LS command */
/// HS to LS ICC command word0
#define H2L_CMD_WORD0                       (ICC_H->icch_hlrw0)
/// HS to LS ICC command word1
#define H2L_CMD_WORD1                       (ICC_H->icch_hlrw1)
/// HS to LS ICC command toggle bit
#define H2L_CMD_TG                          (ICC_H->icch_hptg_b.h2ltg0)
/// HS to LS ICC command ACK interrupt enable
#define H2L_CMD_ACK_IE                      (ICC_H->icch_lptgie_b.l2htgie3)
/// HS to LS ICC command ACK interrupt status
#define H2L_CMD_ACK_IS                      (ICC_H->icch_lptgis_b.l2htgis3)
/// HS to LS ICC command ACK interrupt status mask
#define H2L_CMD_ACK_IS_MASK                 ((uint32_t)(1 << 19))

/* Defines the ICC registers are used for the implementation of the ICC LS to HS command */
/// LS to HS ICC command word0
#define L2H_CMD_WORD0                       (ICC_H->icch_hlrw2)
/// LS to HS ICC command word1
#define L2H_CMD_WORD1                       (ICC_H->icch_hlrw3)
/// LS to HS ICC command Toggle bit
#define L2H_CMD_TG                          (ICC_H->icch_lptg_b.l2htg2)         // L2H command toggle bit, use as 1 bit SN
/// LS to HS ICC command ACK interrupt enable
#define L2H_CMD_IND_IE                      (ICC_H->icch_lptgie_b.l2htgie2)     // L2H Command Indication Interrupt Enable
/// LS to HS ICC command ACK interrupt status
#define L2H_CMD_IND_IS                      (ICC_H->icch_lptgis_b.l2htgis2)     // L2H Command Indication Interrupt Status
/// LS to HS ICC command ACK interrupt status mask
#define L2H_CMD_IND_IS_MASK                 ((uint32_t)(1 << 18))
/// LS to HS ICC command ACK bit
#define L2H_CMD_ACK                         (ICC_H->icch_hptg_b.h2ltg1)

/* Defines the ICC registers are used for the implementation of the ICC message queue */
/// HS to LS ICC message Hi-queue write pointer
#define H2L_HQ_WR_PTR                       (ICC_H->icch_hwlr0)
/// HS to LS ICC message Lo-queue write pointer
#define H2L_LQ_WR_PTR                       (ICC_H->icch_hwlr1)
/// LS to HS ICC message Hi-queue read pointer
#define L2H_HQ_RD_PTR                       (ICC_H->icch_hwlr2)
/// LS to HS ICC message Lo-queue read pointer
#define L2H_LQ_RD_PTR                       (ICC_H->icch_hwlr3)

/// LS to HS ICC message Hi-queue write pointer
#define L2H_HQ_WR_PTR                       (ICC_H->icch_hrlw0)
/// LS to HS ICC message Lo-queue write pointer
#define L2H_LQ_WR_PTR                       (ICC_H->icch_hrlw1)
/// HS to LS ICC message Hi-queue read pointer
#define H2L_HQ_RD_PTR                       (ICC_H->icch_hrlw2)
/// HS to LS ICC message Lo-queue read pointer
#define H2L_LQ_RD_PTR                       (ICC_H->icch_hrlw3)

/// HS to LS ICC message Hi-queue indication toggle bit
#define H2L_HQ_IND_TG                       (ICC_H->icch_hptg_b.hptg0)
/// HS to LS ICC message Lo-queue indication toggle bit
#define H2L_LQ_IND_TG                       (ICC_H->icch_hptg_b.hptg1)
/// LS to HS ICC message Hi-queue indication toggle bit
#define L2H_HQ_ACK_TG                       (ICC_H->icch_hptg_b.hptg2)
/// LS to HS ICC message Lo-queue indication toggle bit
#define L2H_LQ_ACK_TG                       (ICC_H->icch_hptg_b.hptg3)

/// LS to HS ICC message Hi-queue indication interrupt enable
#define L2H_HQ_IND_IE                       (ICC_H->icch_lptgie_b.lptgie0)
/// LS to HS ICC message Hi-queue indication interrupt status
#define L2H_HQ_IND_IS                       (ICC_H->icch_lptgis_b.lptgis0)
/// LS to HS ICC message Hi-queue indication interrupt status mask
#define L2H_HQ_IND_IS_MASK                  ((uint32_t)(1 << 0))
/// LS to HS ICC message Lo-queue indication interrupt enable
#define L2H_LQ_IND_IE                       (ICC_H->icch_lptgie_b.lptgie1)
/// LS to HS ICC message Lo-queue indication interrupt status
#define L2H_LQ_IND_IS                       (ICC_H->icch_lptgis_b.lptgis1)
/// LS to HS ICC message Lo-queue indication interrupt status mask
#define L2H_LQ_IND_IS_MASK                  ((uint32_t)(1 << 1))

/// HS to LS ICC message Hi-queue acknowledge interrupt enable
#define H2L_HQ_ACK_IE                       (ICC_H->icch_lptgie_b.lptgie2)
/// HS to LS ICC message Hi-queue acknowledge interrupt status
#define H2L_HQ_ACK_IS                       (ICC_H->icch_lptgis_b.lptgis2)
/// HS to LS ICC message Hi-queue acknowledge interrupt status mask
#define H2L_HQ_ACK_IS_MASK                  ((uint32_t)(1 << 2))
/// HS to LS ICC message Lo-queue acknowledge interrupt enable
#define H2L_LQ_ACK_IE                       (ICC_H->icch_lptgie_b.lptgie3)
/// HS to LS ICC message Lo-queue acknowledge interrupt status
#define H2L_LQ_ACK_IS                       (ICC_H->icch_lptgis_b.lptgis3)
/// HS to LS ICC message Lo-queue acknowledge interrupt status mask
#define H2L_LQ_ACK_IS_MASK                  ((uint32_t)(1 << 3))

/// HS to LS ICC hardware semaphore update toggle bit
#define H2L_SEMA_TG                         (ICC_H->icch_hptg_b.h2ltgs)

/// HS to LS ICC command acknowledge toggle bit
#define H2L_CMD_ACK_TG                      (ICC_H->icch_lptg_b.l2htg3)

/// @cond DOXYGEN_ROM_HAL_API

/**
 * @addtogroup hs_hal_icc_rom_func ICC HAL ROM APIs.
 * @ingroup hs_hal_icc
 * @{
 */

void hal_icc_enter_critical_rtl8195bhp (hal_icc_adapter_t *icc_adapter);
void hal_icc_exit_critical_rtl8195bhp (hal_icc_adapter_t *icc_adapter);
void hal_icc_reg_irq_rtl8195bhp (irq_handler_t handler);
hal_status_t hal_icc_h2l_cmd_send_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t cmd_word0,
                                            uint32_t cmd_word1, uint32_t timeout_us);
hal_status_t hal_icc_set_queue_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_type,
                                            uint32_t q_start_addr, uint32_t q_size);
hal_status_t hal_icc_set_h2l_hq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_start_addr, uint32_t q_size);
hal_status_t hal_icc_set_h2l_lq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_start_addr, uint32_t q_size);
hal_status_t hal_icc_set_l2h_hq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_start_addr, uint32_t q_size);
hal_status_t hal_icc_set_l2h_lq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_start_addr, uint32_t q_size);
hal_status_t hal_icc_reset_queue_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_id);
void hal_icc_l2h_cmd_ind_hook_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_cmd_ind pcallback, void *pdata);
void hal_icc_l2h_queue_ind_hook_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_q_ind pcallback, void *pdata);
void hal_icc_h2l_queue_ack_hook_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_q_ack pcallback, void *pdata);
void hal_icc_task_yield_hook_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_task_yield task_yield);
void hal_icc_init_rtl8195bhp (hal_icc_adapter_t *icc_adapter);
void hal_icc_deinit_rtl8195bhp (hal_icc_adapter_t *icc_adapter);
hal_status_t hal_icc_push_h2lq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_id_size,
                                            uint8_t *pbuf, uint32_t timeout_us);
uint32_t hal_icc_pull_l2hq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_id,
                                            uint8_t *pbuf, uint32_t size);
BOOL hal_icc_is_queue_empty_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_id);
uint32_t hal_icc_get_queue_free_size_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_id);
hal_status_t hal_icc_push_h2l_hq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint8_t *pbuf,
                                           uint32_t size, uint32_t timeout_us);
hal_status_t hal_icc_push_h2l_lq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint8_t *pbuf,
                                            uint32_t size, uint32_t timeout_us);
uint32_t hal_icc_pull_l2h_hq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint8_t *pbuf, uint32_t size);
uint32_t hal_icc_pull_l2h_lq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint8_t *pbuf, uint32_t size);
hal_status_t hal_icc_send_h2l_hq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint8_t *pbuf, uint32_t size,
                                             uint32_t timeout_us);
hal_status_t hal_icc_send_h2l_lq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint8_t *pbuf, uint32_t size,
                                             uint32_t timeout_us);
hal_status_t hal_icc_send_h2lq_int_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_id, icc_q_rw_req_t *send_req);
hal_status_t hal_icc_send_h2l_hq_int_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_q_rw_req_t *send_req);
hal_status_t hal_icc_send_h2l_lq_int_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_q_rw_req_t *send_req);
uint32_t hal_icc_send_h2lq_abort_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_id);
uint32_t hal_icc_send_h2l_hq_abort_rtl8195bhp (hal_icc_adapter_t *icc_adapter);
uint32_t hal_icc_send_h2l_lq_abort_rtl8195bhp (hal_icc_adapter_t *icc_adapter);
uint32_t hal_icc_recv_l2hq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_id_size,
                                        uint8_t *pbuf, uint32_t timeout_us);
uint32_t hal_icc_recv_l2h_hq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint8_t *pbuf,
                                            uint32_t size, uint32_t timeout_us);
uint32_t hal_icc_recv_l2h_lq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint8_t *pbuf,
                                            uint32_t size, uint32_t timeout_us);
hal_status_t hal_icc_recv_l2hq_int_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_id, icc_q_rw_req_t *rcv_req);
hal_status_t hal_icc_recv_l2h_hq_int_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_q_rw_req_t *rcv_req);
hal_status_t hal_icc_recv_l2h_lq_int_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_q_rw_req_t *rcv_req);
uint32_t hal_icc_recv_l2hq_abort_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t q_id);
uint32_t hal_icc_recv_l2h_hq_abort_rtl8195bhp (hal_icc_adapter_t *icc_adapter);
uint32_t hal_icc_recv_l2h_lq_abort_rtl8195bhp (hal_icc_adapter_t *icc_adapter);
hal_status_t hal_icc_get_sema_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_sema_id_t sema_id);
hal_status_t hal_icc_release_sema_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_sema_id_t sema_id);
hal_status_t hal_icc_ls_write_cmd_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t addr,
                                                uint32_t *pdata, uint32_t len_dw);
hal_status_t hal_icc_ls_read_cmd_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t addr,
                                                uint32_t *pbuf, uint32_t len_dw);
void hal_icc_set_hal_cmd_tbl_rtl8195bhp(hal_icc_adapter_t *icc_adapter, icc_user_cmd_entry_t *pcmd_tbl,
                         uint32_t cmd_tbl_size);
void hal_icc_set_cmd_tbl_rtl8195bhp(hal_icc_adapter_t *icc_adapter, icc_user_cmd_entry_t *pcmd_tbl,
                         uint32_t cmd_tbl_size);
hal_status_t hal_icc_hal_cmd_register_rtl8195bhp(hal_icc_adapter_t *icc_adapter, uint8_t cmd, icc_user_cmd_handler_t callback,
                                        uint32_t cb_arg);
void hal_icc_hal_cmd_unregister_all_rtl8195bhp(hal_icc_adapter_t *icc_adapter);
void hal_icc_hal_cmd_unregister_rtl8195bhp(hal_icc_adapter_t *icc_adapter, uint8_t cmd);
hal_status_t hal_icc_cmd_register_rtl8195bhp(hal_icc_adapter_t *icc_adapter, uint8_t cmd, icc_user_cmd_handler_t callback,
                                        uint32_t cb_arg);
void hal_icc_cmd_unregister_all_rtl8195bhp(hal_icc_adapter_t *icc_adapter);
void hal_icc_cmd_unregister_rtl8195bhp(hal_icc_adapter_t *icc_adapter, uint8_t cmd);
hal_status_t hal_icc_send_msg_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_msg_type_t *picc_msg,
                                             uint32_t timeout_us);
hal_status_t hal_icc_send_msg_int_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_msg_type_t *picc_msg,
                                              icc_user_msg_tx_callback_t callback, uint32_t callback_para);
void hal_icc_msg_queue_ind_hook_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_msg_q_ind_t pcallback);
void hal_icc_set_hal_msg_tbl_rtl8195bhp(hal_icc_adapter_t *icc_adapter, icc_user_msg_entry_t *pmsg_tbl,
                                        uint32_t msg_tbl_size);
void hal_icc_set_msg_tbl_rtl8195bhp(hal_icc_adapter_t *icc_adapter, icc_user_msg_entry_t *pmsg_tbl,
                         uint32_t msg_tbl_size);
hal_status_t hal_icc_hal_msg_register_rtl8195bhp(hal_icc_adapter_t *icc_adapter, uint8_t msg,
                                        icc_user_msg_handler_t callback, uint32_t cb_arg);
void hal_icc_hal_msg_unregister_all_rtl8195bhp(hal_icc_adapter_t *icc_adapter);
void hal_icc_hal_msg_unregister_rtl8195bhp(hal_icc_adapter_t *icc_adapter, uint8_t msg);
hal_status_t hal_icc_msg_register_rtl8195bhp(hal_icc_adapter_t *icc_adapter, uint8_t msg,
                                        icc_user_msg_handler_t callback, uint32_t cb_arg);
void hal_icc_msg_unregister_all_rtl8195bhp(hal_icc_adapter_t *icc_adapter);
void hal_icc_msg_unregister_rtl8195bhp(hal_icc_adapter_t *icc_adapter, uint8_t msg);
void hal_icc_set_timeout_rtl8195bhp(hal_icc_adapter_t *icc_adapter, uint32_t cmd_timeout_us,
                                    uint32_t ls_read_timeout_us);
hal_status_t hal_icc_ping_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t ping_data, uint32_t timeout_us);
hal_status_t hal_icc_ls_write_reg_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t addr, uint32_t value);
uint32_t hal_icc_ls_read_reg_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t addr);
void hal_icc_irq_handler_bh_rtl8195bhp (void);
void hal_icc_init_cmd_q_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint32_t *pbuf, uint8_t cmd_q_size);

// New functions of B-Cut
void hal_icc_msg_data_tx_done_callback (hal_icc_adapter_t *icc_adapter, uint32_t tred_len, uint8_t qid);
void hal_icc_hq_msg_data_tx_done_callback (hal_icc_adapter_t *icc_adapter, uint32_t tred_len);
void hal_icc_lq_msg_data_tx_done_callback (hal_icc_adapter_t *icc_adapter, uint32_t tred_len);
icc_msg_tx_req_t *hal_icc_msg_tx_req_pending_deq (hal_icc_adapter_t *icc_adapter, uint8_t q_id);
hal_status_t hal_icc_msg_hdr_rx_req (hal_icc_adapter_t *icc_adapter, uint8_t qid);
void hal_icc_msg_rx_req_done_inq (hal_icc_adapter_t *icc_adapter, icc_msg_rx_req_t *q_req);
icc_msg_rx_req_t *hal_icc_msg_rx_req_pending_deq (hal_icc_adapter_t *icc_adapter, uint8_t q_id);
void hal_icc_msg_data_rx_callback (hal_icc_adapter_t *icc_adapter, uint32_t tred_len, uint8_t qid);
void hal_icc_msg_hq_data_rx_callback (hal_icc_adapter_t *icc_adapter, uint32_t tred_len);
void hal_icc_msg_lq_data_rx_callback (hal_icc_adapter_t *icc_adapter, uint32_t tred_len);
void hal_icc_msg_hdr_rx_callback (hal_icc_adapter_t *icc_adapter, uint32_t tred_len, uint8_t qid);
void hal_icc_msg_hq_hdr_rx_callback (hal_icc_adapter_t *icc_adapter, uint32_t tred_len);
void hal_icc_msg_lq_hdr_rx_callback (hal_icc_adapter_t *icc_adapter, uint32_t tred_len);
void hal_icc_tx_req_pool_fill_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_msg_tx_req_t *q_req);
picc_msg_tx_req_t hal_icc_tx_req_pool_get_rtl8195bhp (hal_icc_adapter_t *icc_adapter);
hal_status_t hal_icc_msg_tx_req_submit_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_msg_tx_req_t *q_req);
void hal_icc_msg_tx_req_pending_process_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint8_t q_id);
void icc_msg_rx_ready_handler_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint8_t frame_type, uint8_t msg_type,
                                              uint8_t *pmsg_data, uint32_t msg_len);
void hal_icc_msg_rx_req_done_process_rtl8195bh (hal_icc_adapter_t *icc_adapter, uint8_t q_id);
hal_status_t hal_icc_msg_rx_req_submit_rtl8195bhp (hal_icc_adapter_t *icc_adapter, icc_msg_rx_req_t *q_req);
picc_msg_rx_req_t hal_icc_msg_rx_req_done_deq_rtl8195bhp (hal_icc_adapter_t *icc_adapter, uint8_t q_id);
void hal_icc_msg_rx_req_pending_process_rtl8195bh (hal_icc_adapter_t *icc_adapter, uint8_t q_id);
hal_status_t hal_icc_msg_tbl_reg_rtl8195bhp(hal_icc_adapter_t *icc_adapter, uint8_t msg, uint8_t frame,
                                        icc_msg_rx_cb_t callback, uint32_t cb_arg);
void hal_icc_msg_tbl_unreg_rtl8195bhp(hal_icc_adapter_t *icc_adapter, uint8_t msg, uint8_t frame);

/** @} */ /* End of group hs_hal_icc_rom_func */
...
typedef struct hal_icc_func_stubs_s {
    ...
    hal_status_t (*hal_icc_ls_write_cmd) (hal_icc_adapter_t *icc_adapter, uint32_t addr, uint32_t *pdata,
                                          uint32_t len_dw);
    hal_status_t (*hal_icc_ls_read_cmd) (hal_icc_adapter_t *icc_adapter, uint32_t addr, uint32_t *pbuf,
                                         uint32_t len_dw);
    void (*hal_icc_set_hal_cmd_tbl)(hal_icc_adapter_t *icc_adapter, icc_user_cmd_entry_t *pcmd_tbl,
                                    uint32_t cmd_tbl_size);
    ...
    void (*hal_icc_set_hal_msg_tbl)(hal_icc_adapter_t *icc_adapter, icc_user_msg_entry_t *pmsg_tbl,
                                    uint32_t msg_tbl_size);
