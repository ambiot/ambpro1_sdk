...
void hal_gpio_comm_init_rtl8195bhp_patch (phal_gpio_comm_adapter_t pgpio_comm_adp);
hal_status_t hal_gpio_schmitt_ctrl_rtl8195bhp_patch (uint32_t pin_name, BOOLEAN ctrl, uint8_t v_h3l1);
#elif (CHIP_VER == CHIP_B_CUT) && (defined(CONFIG_BUILD_RAM))
hal_status_t hal_gpio_drive_ctrl_rtl8195bhp_patch (uint32_t pin_name, uint8_t drv);
#endif
...
__STATIC_INLINE void hal_gpio_comm_init (phal_gpio_comm_adapter_t pgpio_comm_adp)
{
    ...
    #if (CHIP_VER == CHIP_A_CUT) && (defined(CONFIG_BUILD_RAM))
        hal_gpio_comm_init_rtl8195bhp_patch (pgpio_comm_adp);
    #else
...
__STATIC_INLINE hal_status_t hal_gpio_schmitt_ctrl (uint32_t pin_name, BOOLEAN ctrl, uint8_t v_h3l1)
{
    ...
    #if (CHIP_VER == CHIP_A_CUT) && (defined(CONFIG_BUILD_RAM))
        return hal_gpio_schmitt_ctrl_rtl8195bhp_patch (pin_name, ctrl, v_h3l1);
    #else
...
__STATIC_INLINE hal_status_t hal_gpio_drive_ctrl (uint32_t pin_name, uint8_t drv)
{
        ...
        return hal_gpio_drive_ctrl_rtl8195bhp_patch (pin_name, drv);
    #else
...
#else   // else of "#if !defined(CONFIG_BUILD_SECURE)"
// Secure region
...
#endif  // end of else of #if !defined(CONFIG_BUILD_SECURE)
