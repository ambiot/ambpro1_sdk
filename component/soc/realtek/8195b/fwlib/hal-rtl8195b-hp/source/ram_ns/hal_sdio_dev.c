...
extern void hal_sdio_dev_cmd11_handle (hal_sdio_dev_adapter_t *psdio_adp);
...
hal_status_t hal_sdio_dev_os_init (hal_sdio_dev_adapter_t *psdio_adp, void *tx_task, void *rx_task)
{
    ...
    psdio_adp->os_wait = (void (*)(uint32_t))rtw_msleep_os;
...
hal_status_t hal_sdio_dev_init (void)
{
    ...
    ret = hal_pinmux_register ((io_pin_t *)sdio_dev_pins, PID_SDIOD);
    if (ret != HAL_OK) {
		DBG_SDIO_DEV_ERR ("%s: Pin Mux Err!\r\n", __func__);
        return ret;
    }
    ...
    hal_gpio_schmitt_ctrl (PIN_SDIO_CLK, ON, IO_3p3V);
...
//    g_sdio_adp->dcache_clean_by_addr = hal_cache_stubs.dcache_clean_by_addr;
//    g_sdio_adp->dcache_invalidate_by_addr = hal_cache_stubs.dcache_invalidate_by_addr;
...
//	g_sdio_adp->prxbd_addr = rtw_zmalloc((rx_bd_num * sizeof(sdiod_rxbd_t))+31);
    ...
    ret = hal_sdio_dev_init_hw (g_sdio_adp);
    if (ret != HAL_OK) {
		goto __hal_sdio_dev_init_err;
    }
    // Same priority as SysTick, workaround for nest IRQ issue of A-Cut IC
...
void hal_sdio_dev_deinit (void)
{
    ...
    hal_sdio_dev_deinit_hw (g_sdio_adp);
    ...
    hal_pinmux_unregister ((io_pin_t *)sdio_dev_pins, PID_SDIOD);
    hal_gpio_schmitt_ctrl (PIN_SDIO_CLK, OFF, 1);
}
