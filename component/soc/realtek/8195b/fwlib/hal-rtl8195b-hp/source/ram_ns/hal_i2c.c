...
uint8_t hal_i2c_fltr_qk_cali (hal_i2c_adapter_t *phal_i2c_adapter, uint8_t pin_sel)
{
...
#if 0
                        /* scale it */
                        cap_l_tmp = (rsts_m_cnt+1) * 4;
                        if (rsts_m_cnt > (uint32_t)(HP_I2C_FLTR_RSTS_M_LEN/4)) {
                            DBG_I2C_ERR("Could NOT scale to the quadruple value\n");
                        }

                        for (rsts_m_best_cnt = (rsts_m_cnt+1);
                                ((rsts_m_best_cnt < cap_l_tmp)|| (rsts_m_best_cnt < HP_I2C_FLTR_RSTS_M_LEN)); rsts_m_best_cnt++) {
    ...
    if ((rsts_m_cnt == HP_I2C_FLTR_RSTS_M_LEN) && (cap_m_cnt == HP_I2C_FLTR_CAP_M_LEN)
        ...
        if (rsts_m_best != 0) {
            ...
            if (rsts_m_best_cnt > (uint32_t)(HP_I2C_FLTR_RSTS_M_LEN/2)) {
                DBG_I2C_ERR("Could NOT scale to the double value\n");
            }

            for (cap_m_tmp = (rsts_m_best_cnt+1);
                    ((cap_m_tmp < cap_l_tmp) || (cap_m_tmp < HP_I2C_FLTR_RSTS_M_LEN)); cap_m_tmp++) {
