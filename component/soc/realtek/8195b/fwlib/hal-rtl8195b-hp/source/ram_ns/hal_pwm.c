...
hal_status_t hal_pwm_init (hal_pwm_adapter_t *ppwm_adp, u8 pwm_id, pin_sel_t pin_sel, u16 duty_res_us)
{
    ...
    if ((pwm_id >= MaxPwmNum) || (pin_sel > PinSel3)) {
    ...
    ret = hal_pinmux_register((io_pin_t *)&pin_list[0], PID_PWM0+pwm_id);
    if (ret != HAL_OK) {
...
void hal_pwm_deinit (hal_pwm_adapter_t *ppwm_adp)
{
    ...
    io_pin_t pin_list[2];
        ...
        hal_pwm_deinit_rtl8195bhp_patch (ppwm_adp);
    #else
...
{
    #if (CHIP_VER == CHIP_A_CUT) && (defined(CONFIG_BUILD_RAM))
        return hal_pwm_set_duty_rtl8195bhp_patch (ppwm_adp, period_us, duty_us, start_offset_us);
    #else
...
{
    #if (CHIP_VER == CHIP_A_CUT) && (defined(CONFIG_BUILD_RAM))
        return hal_pwm_set_duty_ns_rtl8195bhp_patch (ppwm_adp, period_ns, duty_ns, start_offset_ns);
    #else
                                    ...
                                    u32 step_sz_ns, u32 step_period_cnt)
{
    //return hal_pwm_stubs.hal_pwm_auto_duty_inc (ppwm_adp, max_duty_us, step_sz_us, step_period_cnt);
    #if (CHIP_VER == CHIP_A_CUT) && (defined(CONFIG_BUILD_RAM))
        return hal_pwm_auto_duty_ns_inc_rtl8195bhp_patch (ppwm_adp, max_duty_ns, step_sz_ns, step_period_cnt);
    #else
...
{
    #if (CHIP_VER == CHIP_A_CUT) && (defined(CONFIG_BUILD_RAM))
        return hal_pwm_auto_duty_ns_dec_rtl8195bhp_patch (ppwm_adp, min_duty_ns, step_sz_ns, step_period_cnt);
    #else
...
{
    #if (CHIP_VER == CHIP_A_CUT) && (defined(CONFIG_BUILD_RAM))
        return hal_pwm_read_duty_ns_rtl8195bhp_patch (ppwm_adp);
    #else
