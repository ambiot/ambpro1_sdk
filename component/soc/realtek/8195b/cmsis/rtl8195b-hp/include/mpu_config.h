...
#define MPU_REGION0_EN                  1   // If MPU region 0 configuration enabled
...
#if defined(CONFIG_VIDEO_RAM)	// Non-cacheable
...
#define MPU_REGION2_BASE                (0x70000000)        /* region base, the address must be aligned to multiple of 32 bytes,
                                         	 	 	 	 		it should sync. with the linker script */
#define MPU_REGION2_LIMIT               (0x705FFFFF)		// region limit, it should sync. with linker script
#else
// MPU region 2 configuration
